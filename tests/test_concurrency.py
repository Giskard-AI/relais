...
from src.relais.base import Stream, Indexed, Index, TaskGroup, EndEvent
import relais as r
...
class TestStreamConcurrency:
    ...
    async def test_concurrent_stream_reading(self):
        ...
        results = await asyncio.gather(reader1_task, reader2_task, return_exceptions=True)
    ...
    async def test_concurrent_stream_writing(self):
        ...
        expected_items = [(100, 10), (101, 20), (200, 30), (201, 40), (300, 50), (301, 60)]
    ...
    async def test_concurrent_sorted_list_calls(self):
        ...
        results = await asyncio.gather(result1_task, result2_task, return_exceptions=True)
        
        # Due to read lock, only one should succeed and get the data
        successful_results = [r for r in results if isinstance(r, list) and r is not None]
...
class TestPipelineConcurrency:
    ...
    async def test_concurrent_pipeline_executions(self):
        ...
        expected_results = [
            ...
            [12, 14, 16, 18, 20],  # [6,7,8,9,10] -> [12,14,16,18,20] -> [12,14,16,18,20]
            []  # [0,1,2] -> [0,2,4] -> []
        ]
    ...
    async def test_nested_concurrent_operations(self):
        ...
        expected = [2, 4, 6, 22, 24, 26, 42, 44, 46]  # Note: 42 should be included (21*2=42 < 50)
    ...
    async def test_concurrent_error_handling(self):
        ...
        error_count = sum(1 for r in results if r == "ERROR" or isinstance(r, Exception))
...
if __name__ == "__main__":
    ...
    async def run_manual_tests():
        ...
        test_classes = [
            ("Stream Concurrency", TestStreamConcurrency),
            ("Pipeline Concurrency", TestPipelineConcurrency), 
            ("TaskGroup Fallback", TestTaskGroupFallback),
            ("Index Concurrency", TestIndexConcurrency)
        ]
