...
class TestAdvancedErrorPropagation:
    ...
    async def test_error_propagation_with_collect_interface(self):
        ...
        pipeline_ignore = r.Pipeline([r.map(failing_function)], error_policy=ErrorPolicy.IGNORE)
    ...
    async def test_nested_pipeline_error_propagation(self):
        ...
        inner_pipeline = r.Pipeline([r.map(inner_failing_function)], error_policy=ErrorPolicy.IGNORE)
        outer_pipeline = r.Pipeline([r.map(outer_failing_function)], error_policy=ErrorPolicy.IGNORE)
    ...
    async def test_error_information_preservation(self):
        ...
        with pytest.raises(Exception) as exc_info:  # Could be PipelineError or the original ValueError
        ...
        if hasattr(exc_info.value, 'exceptions'):
            ...
            for sub_exc in exc_info.value.exceptions:
                ...
                if hasattr(sub_exc, 'original_error') and "Error in step2 for x=6" in str(sub_exc.original_error):
        ...
        assert "error" in error_message.lower() or "exception" in error_message.lower(), f"Expected some error information, got: {error_message}"
    ...
    async def test_resource_cleanup_on_error(self):
        ...
        pipeline = r.Pipeline([r.map(resource_processor)], error_policy=ErrorPolicy.IGNORE)
    ...
    async def test_error_aggregation_in_collect_policy(self):
        ...
        pipeline = r.Pipeline([r.map(multi_error_function)], error_policy=ErrorPolicy.COLLECT)
    ...
    async def test_concurrent_error_isolation(self):
        ...
        pipeline = r.Pipeline([r.map(tracking_processor)], error_policy=ErrorPolicy.IGNORE)
    ...
    async def test_cascading_error_prevention(self):
        ...
        assert stage_errors["stage3"] == 0  # No items reached this stage with error conditions
...
class TestErrorRecoveryPatterns:
    ...
    async def test_retry_pattern_with_error_recovery(self):
        ...
        async def flaky_processor(x):
            ...
            if x == 3 and attempt_counts[x] < 3:
                raise ValueError(f"Transient error for x={x}, attempt {attempt_counts[x]}")
    ...
    async def test_circuit_breaker_pattern(self):
        ...
        pipeline = r.Pipeline([r.map(circuit_breaker_processor)], error_policy=ErrorPolicy.IGNORE)
    ...
    async def test_bulkhead_isolation_pattern(self):
        ...
        critical_pipeline = r.Pipeline([r.map(critical_processor)], error_policy=ErrorPolicy.FAIL_FAST)
        non_critical_pipeline = r.Pipeline([r.map(non_critical_processor)], error_policy=ErrorPolicy.IGNORE)
