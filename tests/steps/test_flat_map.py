...
class TestFlatMap:
    ...
    async def test_sync_flat_map(self):
        ...
        pipeline = r.flat_map(lambda x: [x, x * 2])
    ...
    async def test_async_flat_map(self):
        ...
        pipeline = r.flat_map(async_duplicate)
        result = await ([1, 2, 3] | pipeline).collect()
    ...
    async def test_flat_map_empty_results(self):
        ...
        pipeline = r.flat_map(conditional_expand)
    ...
    async def test_flat_map_single_item_expansion(self):
        ...
        pipeline = r.flat_map(lambda x: [x] * x)  # Repeat each item x times
        result = await ([1, 2, 3] | pipeline).collect()
    ...
    async def test_flat_map_with_strings(self):
        ...
        pipeline = r.flat_map(lambda word: list(word))  # Split word into characters
    ...
    async def test_flat_map_preserves_order(self):
        ...
        pipeline = r.flat_map(slow_for_even)
    ...
    async def test_flat_map_nested_structure_flattening(self):
        ...
        data = [[1, 2], [3, 4], [5]]
        pipeline = r.flat_map(lambda sublist: sublist)  # Flatten one level
