...
from ..base import Step, Stream, StatelessStreamProcessor, Indexed, T, ErrorPolicy
...
class _AsyncIteratorProcessor(StatelessStreamProcessor[None, T]):
    ...
    def __init__(self, input_stream: Stream[None], output_stream: Stream[T], async_iter: AsyncIterator[T]):
    ...
    async def process_stream(self):
        ...
        try:
            ...
            await self.output_stream.handle_error(e, Indexed(-1, None).index, self.__class__.__name__)
        finally:
            await self._cleanup()
...
class AsyncIteratorStep(Step[None, T]):
    ...
    def _build_processor(self, input_stream: Stream[None], output_stream: Stream[T]) -> _AsyncIteratorProcessor[T]:
